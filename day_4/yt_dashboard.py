...
def load_data():
    """
    Loads the data from the csv files and returns dataframes

    :return:
    """

    agg_metrics_by_video_file_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "data", "Aggregated_Metrics_By_Video.csv")
    )
    # Removing the first row that has total data
    _df_agg = pd.read_csv(agg_metrics_by_video_file_path).iloc[1:, :]
    _df_agg.columns = [
        "Video",
        "Video title",
        "Video publish time",
    ...
    _df_agg["Video publish time"] = pd.to_datetime(_df_agg["Video publish time"])
    _df_agg["Average view duration"] = _df_agg["Average view duration"].apply(
        lambda x: datetime.strptime(x, "%H:%M:%S")
    )
    _df_agg["Avg_duration_sec"] = _df_agg["Average view duration"].apply(
        lambda x: x.second + x.minute * 60 + x.hour * 3600
    )
    _df_agg["Engagement_ratio"] = (
        _df_agg["Comments added"] + _df_agg["Shares"] + _df_agg["Dislikes"] + _df_agg["Likes"]
    ) / _df_agg.Views
    _df_agg["Views / sub gained"] = _df_agg["Views"] / _df_agg["Subscribers gained"]
    _df_agg.sort_values("Video publish time", ascending=False, inplace=True)

    agg_metrics_by_country_and_sub_status_file_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "data", "Aggregated_Metrics_By_Country_And_Subscriber_Status.csv")
    )
    _df_agg_sub = pd.read_csv(agg_metrics_by_country_and_sub_status_file_path)

    comments_file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "data", "All_Comments_Final.csv"))
    _df_comments = pd.read_csv(comments_file_path)

    video_performance_file_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "data", "Video_Performance_Over_Time.csv")
    )
    _df_time = pd.read_csv(video_performance_file_path)
    _df_time["Date"] = pd.to_datetime(_df_time["Date"])

    return _df_agg, _df_agg_sub, _df_comments, _df_time
...
df_time_diff = pd.merge(
    df_time, df_agg.loc[:, ["Video", "Video publish time"]], left_on="External Video ID", right_on="Video"
)
...
if add_sidebar == "Aggregate Metrics":
    ...
    st.dataframe(
        df_agg_diff_final.style.applymap(style_negative, props="color:red")
...
if add_sidebar == "Individual Video Analysis":
    ...
    fig2.update_layout(
        title="View comparison first 30 days", xaxis_title="Days Since Published", yaxis_title="Cumulative views"
    )
